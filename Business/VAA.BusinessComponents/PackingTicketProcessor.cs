...
namespace VAA.BusinessComponents
{
    ...
    public class PackingTicketProcessor : IPackingTicket
    {
        ...
        public List<BoxTicketSortedData> CreateBoxTicketProofs(long orderId)
        {
            try
            {
                //delete the proofs template first
                //_orderManagement.DeleteBoxTicketProof(orderId);
                ...
                foreach (var boxTicket in SortedBoxTicketDataList)
                {
                    ...
                    if (!string.IsNullOrEmpty(boxTicketTempalte.ChiliDocumentID))
                        continue;

                    //create chilidocument
                    var chilidoc = chili.CreateChiliDocumentForPackingTicket(InstanceId, boxTicket.ID, boxTicketTempalte.TemplateID, Convert.ToString(boxTicket.ID));
                    _orderManagement.UpdateBoxTemplate(boxTicket.ID, boxTicketTempalte.TemplateID, chilidoc);
                    ...
                    values.Add("Date", Convert.ToDateTime(boxTicket.Date).ToShortDateString());
        ...
        public void CreateBoxTicketPDfs(long orderId, List<BoxTicketSortedData> sortedBoxTicketData)
        {
            try
            {
                if (sortedBoxTicketData == null || sortedBoxTicketData.Count == 0)
                    return;
                ...
                try
                {
                    if (!Directory.Exists(pdfInputDestination))
                        Directory.CreateDirectory(pdfInputDestination);

                }
                catch { }
                try
                {
                    if (!Directory.Exists(pdfOutputDestination))
                        Directory.CreateDirectory(pdfOutputDestination);
                }
                catch { }

                List<byte[]> filesByte = new List<byte[]>();

                var tasks = new List<tPDFGenerationTasksPackingTicket>();

                var boxTicketData = _orderManagement.GetBoxTicketData(orderId);

                foreach (var boxTicket in sortedBoxTicketData)
                {
                    //createPDF - once proof is done
                    var ticketTemplate = _orderManagement.GetBoxTicketTemplate(boxTicket.ID);

                    var task = chili.AddPdfGenerationTaskForBoxTicketTemplate(ticketTemplate);

                    tasks.Add(task);

                    GenerateSinglePackingTicketPDFs(PdfGenerationJobId, boxTicket.ID);
                }
                ...
                foreach (var task in tasks)
                {
                    try
                    {
                        if (!string.IsNullOrEmpty(task.LocalPDFFile))
                        {
                            var filePath = System.IO.Path.Combine(pdfInputDestination, task.LocalPDFFile);
