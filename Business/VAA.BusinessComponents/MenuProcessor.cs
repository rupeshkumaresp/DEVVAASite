...
namespace VAA.BusinessComponents
{
    ...
    public class MenuProcessor : IMenuProcessor
    {
        ...
        bool foodGuideRecent = Convert.ToBoolean(System.Configuration.ConfigurationManager.AppSettings["FoodGuideRecentPDF"]);
        bool nextCycleProcessPDF = Convert.ToBoolean(System.Configuration.ConfigurationManager.AppSettings["nextCycleProcessPDF"]);
        ...
        /// <summary>
        /// Bulk Base Item import from excel sheet
        /// </summary>
        /// <param name="stream"></param>
        public void ImportBaseMenuItems(Stream stream)
        {
            ...
            foreach (var dataSetName in importer.GetDataSetNames())
            {
                if (worksheet == 0)
                {
                    ...
                    if (importedRows != null && importedRows.Any())
                    {
                        ...
                        {
                            var className = Convert.ToString(row["class"]).Trim();
                            var menuType = Convert.ToString(row["menu type"]).Trim();
                            var category = Convert.ToString(row["category"]).Trim();
                            var title = Convert.ToString(row["title"]);
                            var serviceCode = Convert.ToString(row["service code"]).Trim();
                            var titleDescription = Convert.ToString(row["title description"]);
                            var description = Convert.ToString(row["description"]);
                            var subDescription = Convert.ToString(row["sub description"]);
                            var attributes = Convert.ToString(row["attributes"]);
                            ...
                            int classId = _menuManagement.GetClassIdbyName(className);
                            var categoryDetails = _menuManagement.GetMenuItemCategory(category, 1);

                            if (categoryDetails == null)
                                continue;

                            int menuTypeId = 1;
                            long categoryId = categoryDetails.ID;

                            var menuname = "";

                            if (classId == 1)
                                menuname = className + " Class " + menuType;
                            else
                                menuname = className + " " + menuType;

                            menuTypeId = _menuManagement.GetMenuTypeByMenuName(menuname);

                            BaseItem baseItem = new BaseItem()
                            {
                                BaseItemCode = serviceCode,
                                ClassId = classId,
                                MenuTypeId = menuTypeId,
                                LanguageId = 1,
                                CategoryId = categoryId,
                                BaseItemTitle = title,
                                BaseItemTitleDescription = titleDescription,
                                BaseItemDescription = description,
                                BaseItemSubDescription = subDescription,
                                BaseItemAttributes = attributes,
                            };

                            if (!_baseItemManagement.ExistingBaseItem(serviceCode))
                                _baseItemManagement.CreateNewBaseItem(baseItem);
                            else
                                _baseItemManagement.UpdateBaseItem(baseItem);
                        }
        ...
        private static string GenerateMenuCode(int menuTypeId, string menuCode)
        {
            var definingChar = "D";
        ...
        public void CreateChiliDocumentForReOrderMenu(List<long> menuIdCollection)
        {
            for (int i = 0; i < menuIdCollection.Count; i++)
            {
                ...
                CreateChiliDocumentForReOrderMenuBymenuid(menuId);
            }
        }

        public void CreateChiliDocumentForReOrderMenuBymenuid(long menuId)
        {
        ...
        public void GeneratePdfForOrder(long cycleId, int classId, long orderId)
        {
            try
            {
                ...
                pdfInputDestination = EmmaPDFPathFolder + @"\MENU PDFS\" + +orderId;
                pdfOutputDestination = EmmaPDFPathFolder + @"\MENU PDFS\ " + orderId + @"\Output";
                        ...
                        try
                        {
                            ...
                            if (menu.LanguageId != 1)
                            {
                                try
                                {
                                    ...
                                    var outfilePath = System.IO.Path.Combine(pdfInputDestination, fileName);
        ...
        public void Generate(long cycleId, int jobID, long menuID, int templateID, int languageId)
        {
            ...
            if (string.IsNullOrEmpty(pdfInputDestination))
            {
                pdfInputDestination = EmmaPDFPathFolder + @"\MENU PDFS\";
                pdfOutputDestination = EmmaPDFPathFolder + @"\MENU PDFS\Output";
            ...
            if (!exists)
                System.IO.Directory.CreateDirectory(pdfInputDestination);
        ...
        public bool FlightScheduleValidityCheck(long liveOrderId, DateTime fromDate, DateTime toDate)
        {
            bool valid = true;
            var liveOrderDetails = _orderManagement.GetLiveOrderDetails(liveOrderId);

            //get lowest effective date
            var effectiveDate = Convert.ToDateTime(_routeManagement.GetLowerEffectiveDate(""));

            //get highest discontinue date
            var discontinuedDate = Convert.ToDateTime(_routeManagement.GetUpperDiscontinueDate(""));

            if (!(fromDate >= effectiveDate && fromDate <= discontinuedDate && toDate >= effectiveDate && toDate <= discontinuedDate))
            {
                valid = false;
        ...
        public void CalculateQuantity(long liveOrderId, DateTime fromDate, DateTime toDate)
        {
            ...
            try
            {
                ...
                foreach (var orderDetails in liveOrderDetails)
                {
                    ...
                    var isFoodGuide = false;

                    if (menu.MenuTypeId == 5)
                        isFoodGuide = true;

                    var foodGuideEquipmentType = "744-LHRE";
                    ...
                    if (classId == 1)
                    {
                        if (isWednesday)
                        {
                            if (isFoodGuide && flightScheduleWednesday.EquipmentType == foodGuideEquipmentType)
                                totalQuantity += 3 * _routeManagement.UpperClassCapacity(flightScheduleWednesday.EquipmentType);
                            else
                        ...
                        if (isThursday)
                        {
                            if (isFoodGuide && flightScheduleThursday.EquipmentType == foodGuideEquipmentType)
                                totalQuantity += 3 * _routeManagement.UpperClassCapacity(flightScheduleThursday.EquipmentType);
                            else
                        ...
                        if (isFriday)
                        {
                            if (isFoodGuide && flightScheduleFriday.EquipmentType == foodGuideEquipmentType)
                                totalQuantity += 3 * _routeManagement.UpperClassCapacity(flightScheduleFriday.EquipmentType);
                            else
                        ...
                        if (isSaturday)
                        {
                            if (isFoodGuide && flightScheduleSaturday.EquipmentType == foodGuideEquipmentType)
                                totalQuantity += 3 * _routeManagement.UpperClassCapacity(flightScheduleSaturday.EquipmentType);
                            else
                        ...
                        if (isSunday)
                        {
                            if (isFoodGuide && flightScheduleSunday.EquipmentType == foodGuideEquipmentType)
                                totalQuantity += 3 * _routeManagement.UpperClassCapacity(flightScheduleSunday.EquipmentType);
                            else
                        ...
                        if (isMonday)
                        {
                            if (isFoodGuide && flightScheduleMonday.EquipmentType == foodGuideEquipmentType)
                                totalQuantity += 3 * _routeManagement.UpperClassCapacity(flightScheduleMonday.EquipmentType);
                            else
                        ...
                        if (isTuesday)
                        {
                            if (isFoodGuide && flightScheduleTuesday.EquipmentType == foodGuideEquipmentType)
                                totalQuantity += 3 * _routeManagement.UpperClassCapacity(flightScheduleTuesday.EquipmentType);
                            else
