...
namespace VAA.DataAccess
{
    ...
    public class RouteManagement : IRoute
    {
        ...
        public DateTime? GetLowerEffectiveDate(string flightNo)
        {
            return (from fs in _context.tFlightSchedule /*where fs.FlightNo == flightNo*/ select fs.Effective).Min();
        }

        public DateTime? GetUpperDiscontinueDate(string flightNo)
        {
            return (from fs in _context.tFlightSchedule /*where fs.FlightNo == flightNo*/ select fs.Discontinued).Max();
        }
